import { call, put, takeEvery } from "redux-saga/effects";
import API from "../../../helper/api";
import { PUT_CONTENT, GET_CONTENT } from "./actionTypes";
import { loadSuccess, loadContent, apiError } from "./actions";

function* saveContent({ payload: data }) {
  try {
    const response = yield call(API.post, `/will`, data);
    const { success, result } = response.data;
    if (success) {
      localStorage.setItem("passdownData", result);
    }
    yield put(loadSuccess(response));
  } catch (error) {
    yield put(apiError(error));
  }
}

function* getContent({ payload: email }) {
  try {
    const response = yield call(API.get, `/will/${email}`);
    const { agree } = response.data;
    yield put(loadContent({ agree }));
  } catch (error) {
    yield put(apiError(error));
  }
}

function* passdownSaga() {
  yield takeEvery(PUT_CONTENT, saveContent);
  yield takeEvery(GET_CONTENT, getContent);
}

export default passdownSaga;
